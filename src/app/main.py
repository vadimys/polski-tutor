...
from app.bot.event_mw import EventMiddleware
...
COMMANDS = [
    ...
    BotCommand(command="gramatyka", description="📚 Граматика (курс з нуля)"),
    BotCommand(command="czasowniki", description="🔀 Дієслова (довідник + тренажер)"),
    ...
    BotCommand(command="przyporzadkowanie", description="Аудіо-зіставлення (мовці↔описи)"),
    BotCommand(command="formy", description="Впиши форму (граматика)"),
    BotCommand(command="transformacje", description="Трансформація речень (граматика)"),
...
async def main() -> None:
    ...
    dp.message.middleware(EventMiddleware())  # трекінг використання фіч (аналітика)
    dp.callback_query.middleware(EventMiddleware())
    ...
    for r in (
        ...
        audiomatch, examiner, teacher, support, plan, grammar, grammar_verbs, menu,
    ):
