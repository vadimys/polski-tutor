...
async def feedback(task: SpeakTask, transcript: str) -> tuple[str, tuple[int, int, int] | None]:
    ...
    try:  # сирі бали; клемпінг — у readiness_pct
        ...
        scores = None
    from app.services.eval_feedback import speaking_contract  # відкладено — уникаємо циклів

    bounds = [(0, task.max_wykonanie), (0, 8), (0, 8)]
    issues = speaking_contract(fb, scores, bounds)  # моніторинг якості (лог, не блокує учня)
    if issues:
        logger.warning("speaking feedback contract issues %s (task=%s)", issues, task.id)
    return fb, scores
