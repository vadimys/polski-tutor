...
_SYSTEM = (
    ...
    "1. Витягни РІВНО той польський текст, що на фото. Це друкований текст із книжки, тож кожне "
    "слово — СПРАВЖНЄ польське слово, а речення мають сенс. Читай уважно: НЕ плутай схожі слова "
    "(напр. ukradła≠układała) і ОБОВʼЯЗКОВО збережи всі діакритики (ą ć ę ł ń ó ś ź ż) — "
    "wiedźma, dźwięk, łabędź, niedźwiedź, znajdź. Якщо через кут/розмиття літера нечітка — обери "
    "той варіант, що дає правильне польське слово. Не вигадуй і не додавай речень, яких немає. "
    "Ігноруй сторонні написи (номери сторінок, підписи до картинок, службові позначки «dź Dź»).\n"
