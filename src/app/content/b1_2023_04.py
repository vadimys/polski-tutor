...
_ZAD3 = MatchTask(
    ...
    explain=[
        "«według zasady ___» → F (мистецтво для кожного).",
        "«niektórych materiałów ___» → B (можна й послухати в аудіо).",
        "«prezentować inaczej, ___» → A (без складних наукових термінів).",
        "«oraz ___» → D (інформація про події зі світу мистецтва).",
        "«pisarze, ___» → G (але всіх єднає одна риса).",
        "«katalog, który ___» → E (буде доступний кожному й будь-коли).",
        "«nie można obejrzeć, ___» → C (бо сховані в музейних сховищах)."],
)
...
_ZAD4 = MatchTask(
    ...
    explain=[
        "не будь пасивним → D (запропонуй допомогу на вулиці).",
        "друге життя речам → G (ковдри/миски — до притулку).",
        "збірка в праці → A (колеги збирають продукти).",
        "пакунок для дітей → F (домдитини: іграшки, книжки).",
        "передай найцінніше → B (здай кров).",
        "відмовся від подарунків → E (нехай перекажуть на благодійність).",
        "стань волонтером → C (даруй свій час і залучення)."],
)
