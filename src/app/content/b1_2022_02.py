...
_ZAD3 = MatchTask(
    ...
    explain=[
        ...
        "«w głębokim basenie po to, ___» → E (щоб не діставати ногами дна).",
        "«Dla bezpieczeństwa ___» → G (кожен має спеціальний одяг).",
        "«nordic walking, ___» → A (тобто популярні прогулянки з палицями).",
        "«w lasach lub parkach, ___» → F (тобто на свіжому повітрі).",
        "«nie wszyscy wiedzą, ___» → D (що це теж вид спорту).",
        "«osób w naszym wieku, ___» → H (тому варто записатися до клубу сеньйора)."],
)
...
_ZAD4 = MatchTask(
    ...
    explain=[
        "нащо середня школа → F (важливий етап, шлях до кар'єри).",
        "не рішення мами чи тата → G (це ТВІЙ вибір).",
        "після уроків → H (гуртки, майстерні, конкурси поза програмою).",
        "чи важлива думка про школу → A (не орієнтуйся на рейтинги/престиж).",
        "найважливіше в освіті → B (що для тебе має значення).",
        "який тип школи → E (ліцей, технікум, галузева).",
        "дорога до школи → D (зручно мати школу поряд)."],
)
