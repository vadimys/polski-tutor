...
_ZAD4 = MatchTask(
    ...
    explain=[
        "уникай сезону відпусток → G (пік сезону — ціни вищі).",
        "не купуй у вихідні → E (усі шукають у вихідні → дорожче).",
        "підпишись на розсилку → A (email-знижки).",
        "стережись доплат → C (додаткові послуги поза ціною).",
        "обирай великі міста → D (велика конкуренція → промоції).",
        "порівнюй, не мисли стереотипами → B (дешеве не завжди дешеве).",
        "перевір валюту переказу → F (конвертація через долар)."],
)
