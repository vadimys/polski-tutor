...
_ZAD3 = MatchTask(
    ...
    explain=[
        ...
        "«zaczął w Glasgow, ___ w Cambridge» → G (але магістратуру — в університеті).",
        "«praktyki…, ___» → E (які справили величезне враження).",
        "«firmę…, ___» → D (яка хоче допомагати майбутнім студентам).",
        "«Okazuje się, ___, tym łatwiej» → C (що чим раніше сплануємо).",
        "«z kandydatem, ___» → B (щоб побудувати точний профіль).",
        "«mają znaczenie, ___» → F (але важливіше — позаакадемічне)."],
)
...
_ZAD4 = MatchTask(
    ...
    explain=[
        "відклади смартфон → E (не носи весь час при собі).",
        "потоваришуй з тишею → A (вимкни звук і сповіщення).",
        "не роби багато нараз → D (працюєш — зосередься на цьому).",
        "знайди хобі → F (без хобі легше залежність).",
        "дбай про сон → C (годину до сну без телефону).",
        "дбай про дружбу → B (реальні контакти захищають).",
        "плануй перерви → G (цифровий детокс, час поза мережею)."],
)
