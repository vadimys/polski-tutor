...
_ZAD3 = MatchTask(
    ...
    explain=[
        "«sprawy zawodowe ___» → G (та дбаємо про здоров'я).",
        "«Arystoteles odkrył ___» → H (позитивний вплив ходьби на мислення).",
        "«ideę, ___» → F (яка вже популярна у США).",
        "«Założyciel… ___ w czasie spacerów» → A (перших партнерів пізнавав).",
        "«zwiększają efekty, ___» → D (бо легше зосередитися).",
        "«Siedzący tryb życia ___» → C (дуже шкідливий для організму).",
        "«osoba, ___, powinna spacerować» → E (яка працює в офісі)."],
)
...
_ZAD4 = MatchTask(
    ...
    explain=[
        "шукає вади → F (довго сперечається про ціну, колір, функції).",
        "колекціонер → G (ходить по безкоштовні зразки).",
        "любить повернення → A (одразу знає, що не для нього, поверне).",
        "знає все → D (усе прочитав, інструкції напам'ять).",
        "любить гуляти → H (20 хв повільно ходить, іде з порожньою торбою).",
        "має багато грошей → C (важливо, щоб було найкраще, ціна неважлива).",
        "шукає знижки → E (приходить на розпродажі)."],
)
