...
_ZAD3 = MatchTask(
    ...
    explain=[
        "«potraktujesz ___» → F (як інші обов'язки).",
        "«zaplanować ___» → A (і записати в календарі).",
        "«dopiero zaczynasz, ___» → D (тренуйся регулярно, через день).",
        "«raz na 7 dni, ___» → G (не чекай видовищних ефектів).",
        "«prawdopodobnie ___» → B (швидко набриднуть).",
        "«do kontuzji, ___» → C (тож краще почни із наборів вправ).",
        "«Takie ćwiczenia ___» → E (допоможуть зміцнити все тіло)."],
)
...
_ZAD4 = MatchTask(
    ...
    explain=[
        "місячна винагорода → C (таблиця обов'язків і сум, скільки дитина заробить).",
        "не тільки гроші за працю → F (молодших нагороджуй купонами).",
        "навчи ділитися → D (частину заощаджень — на благодійність).",
        "почни з роботи над собою → B (ти взірець, зміни свої звички).",
        "розмовляй про витрати → A (пояснюй дітям сімейний бюджет).",
        "навчи заощаджувати на приємності → E (баночки з цілями)."],
)
