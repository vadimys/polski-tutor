...
_ZAD3 = MatchTask(
    ...
    explain=[
        "«problemy lokalowe, ___» → D (які зараз мають молоді поляки).",
        "«atrakcyjnych lokalizacjach, ___» → A (напр. біля університетів).",
        "«blisko miasta, ___ takimi mieszkaniami» → G (тож завжди будуть зацікавлені).",
        "«z Ameryki, ___» → B (а конкретно з Нью-Йорка).",
        "«na początku XX wieku, ___ we Wrocławiu» → F (а перші мікрокавалерки постали).",
        "«do osób, ___ dla dziecka» → C (які планували купити житло).",
        "«blisko centrum Warszawy, ___ 18 000 zł» → E (де квадратний метр коштує).",
    ],
)
...
_ZAD4 = MatchTask(
    ...
    explain=[
        "обов'язки й витрати → C (корм, ветеринар, щеплення).",
        "збіг характерів → G (обрати тип пса під стиль життя родини).",
        "самотні миті → A (доросле може лишатися саме, якщо привчати).",
        "прогулянка й у негоду → F (пес не лишається вдома через холод/дощ).",
        "порядок мусить бути → B (більше часу на прибирання).",
        "спільний відпочинок → E (готелі, куди можна з твариною).",
        "виховання з перших днів → D (від початку встановити правила)."
    ],
)
